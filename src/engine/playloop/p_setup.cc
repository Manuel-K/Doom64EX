    ...
    while(sc_parser.readtokens()) {
        ...
        if(!dstricmp(sc_parser.token, "MAP")) {
            ...
            while(sc_parser.readtokens()) {
                ...
                if(!sc_parser.setdata(reinterpret_cast<byte*>(&mapdef), mapdatatable)) {
                    dboolean ok = false;
        ...
        //
            ...
            while(sc_parser.readtokens()) {
                ...
                if(!sc_parser.setdata(reinterpret_cast<byte*>(&cluster), clusterdatatable)) {
                    char* text;
    ...
    while(sc_parser.readtokens()) {
        ...
        if(!dstricmp(sc_parser.token, "SKY")) {
            ...
            while(sc_parser.readtokens()) {
                ...
                if(!sc_parser.setdata(reinterpret_cast<byte*>(&sky), skydatatable)) {
                    //
                    // check for sky flags
                    //
