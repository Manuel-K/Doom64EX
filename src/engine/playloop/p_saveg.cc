...
static void saveg_setup_mobjread(void) {
    ...
    for(i = 0; i < savegmobjnum; i++) {
        savegmobj[i].index = i + 1;
        savegmobj[i].mobj = reinterpret_cast<mobj_t*>(Z_Calloc(sizeof(mobj_t), PU_LEVEL, nullptr));
    }
}
...
static void saveg_read_mobj_t(mobj_t* mo) {
    ...
    pl                  = saveg_read32();
    mo->player          = pl > 0 ? &players[pl - 1] : nullptr;
    ...
    mo->mobjfunc        = saveg_read32() ? P_RespawnSpecials : nullptr;
}
...
static void saveg_read_pspdef_t(pspdef_t *psp) {
    ...
    psp->state      = state > 0 ? &states[state] : nullptr;
...
static void saveg_write_ceiling_t(ceiling_t* ceiling) {
    ...
    saveg_write32(ceiling->thinker.function.acp1 != nullptr ? 1 : 0);
}
...
static void saveg_write_plat_t(plat_t* plat) {
    ...
    saveg_write32(plat->thinker.function.acp1 != nullptr ? 1 : 0);
}
...
static void saveg_read_delay_t(delay_t* delay) {
    ...
    func                = saveg_read32();
    delay->finishfunc   = func ? G_CompleteLevel : nullptr;
}
...
static void saveg_read_aimcamera_t(aimcamera_t* cam) {
    ...
    pl              = saveg_read32();
    cam->activator  = pl > 0 ? &players[pl - 1] : nullptr;
}
...
static void saveg_read_movecamera_t(movecamera_t* cam) {
    ...
    pl              = saveg_read32();
    cam->player     = pl > 0 ? &players[pl - 1] : nullptr;
...
static void saveg_write_laserthinker_t(laserthinker_t* laserthinker) {
    ...
    for(l = laserthinker->laser; l != nullptr; l = l->next) {
        saveg_write32(l->x1);
        saveg_write32(l->y1);
        saveg_write32(l->z1);
        saveg_write32(l->x2);
...
static void saveg_read_laserthinker_t(laserthinker_t* laserthinker) {
    ...
    while(1) {
        if(head) {
            head = false;
            laserthinker->laser = (laser_t*)Z_Calloc(sizeof(laser_t), PU_LEVSPEC, nullptr);
            l = laserthinker->laser;
        }
        else {
            l->next = (laser_t*)Z_Calloc(sizeof(laser_t), PU_LEVSPEC, nullptr);
            l = l->next;
        }
    ...
    while(1) {
        ...
        for(i = 0; saveg_specials[i].type != tc_endthinkers; i++) {
            if(tclass == saveg_specials[i].type) {
                saveg_read_pad();
                thinker = Z_Malloc(saveg_specials[i].structsize, PU_LEVEL, nullptr);
                ...
                case tc_ceiling:
                    ...
                    if(!specialthinker) {
                        ((thinker_t*)thinker)->function.acp1 = nullptr;
                    }
                ...
                if(((thinker_t*)thinker)->function.acp1 != nullptr) {
                    specialthinker = saveg_read32();
                    if(specialthinker) {
