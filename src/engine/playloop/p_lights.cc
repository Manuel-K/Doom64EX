...
void P_SpawnSequenceLight(sector_t* sector, dboolean first) {
    ...
    seq = reinterpret_cast<sequenceGlow_t*>(Z_Malloc(sizeof(*seq), PU_LEVSPEC, 0));
...
void T_Combine(combine_t *combine) {
    ...
    if(combine->special != sector->special ||
       combine->func.acp1 != combine->combiner->function.acp1) {
        sector->lightlevel = 0;
...
void P_UpdateLightThinker(light_t* destlight, light_t* srclight) {
    ...
    lt = reinterpret_cast<lightmorph_t*>(Z_Malloc(sizeof(*lt), PU_LEVSPEC, 0));
...
void P_FadeInBrightness(void) {
    ...
    fb = reinterpret_cast<fadebright_t*>(Z_Malloc(sizeof(*fb), PU_LEVSPEC, 0));
