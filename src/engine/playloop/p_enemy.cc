...
static mobj_t *P_MissileAttack(mobj_t *actor, int direction)
{
    ...
    mo = P_SpawnMissile(actor, actor->target, static_cast<mobjtype_t>(type), deltax, deltay, deltaz, aim);
...
void A_CyberDeathEvent(mobj_t *actor)
{
    ...
    exp = reinterpret_cast<mobjexp_t *>(Z_Calloc(sizeof(*exp), PU_LEVSPEC, 0));
