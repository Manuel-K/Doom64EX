...
int GL_BindGfxTexture(const char* name, dboolean alpha) {
    ...
    SetTextureImage(reinterpret_cast<byte*>(Image_GetData(image)), (alpha ? 4 : 3), &width, &height, format, type);
    Image_Free(image);
...
void GL_BindSpriteTexture(int spritenum, int pal) {
    ...
    SetTextureImage(reinterpret_cast<byte*>(Image_GetData(image)), 4, &w, &h, GL_RGBA8, GL_RGBA);
    Image_Free(image);
