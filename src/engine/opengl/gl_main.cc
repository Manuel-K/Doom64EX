...
static dboolean FindExtension(const char *ext) {
    ...
    start = extensions;
    for(;;) {
        where = (byte *)strstr(reinterpret_cast<const char*>(start), ext);
        if(!where) {
            break;
        }
...
void GL_Init(void) {
    gl_vendor = reinterpret_cast<const char*>(dglGetString(GL_VENDOR));
    I_Printf("GL_VENDOR: %s\n", gl_vendor);
    gl_renderer = reinterpret_cast<const char*>(dglGetString(GL_RENDERER));
    I_Printf("GL_RENDERER: %s\n", gl_renderer);
    gl_version = reinterpret_cast<const char *>(dglGetString(GL_VERSION));
