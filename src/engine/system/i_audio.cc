...
#endif
{
    std::string driver = cvar->string;

    // this is absolutely horrible
    if(cvar->defvalue == "default") {
...
void I_InitSequencer(void) {
    ...
    fluid_settings_setstr(doomseq.settings, "audio.driver", s_driver.string.c_str());

    CON_DPrintf("Audio driver: %s\n", s_driver.string.c_str());
    ...
    if (s_soundfont.string[0]) {
        if (I_FileExists(s_soundfont.string.c_str())) {
            I_Printf("Found SoundFont %s\n", s_soundfont.string.c_str());
            doomseq.sfont_id = fluid_synth_sfload(doomseq.synth, s_soundfont.string.c_str(), 1);

            CON_DPrintf("Loading %s\n", s_soundfont.string.c_str());

            sffound = true;
        } else {
            CON_Warnf("CVar s_soundfont doesn't point to a file.", s_soundfont.string.c_str());
        }
    }
