...
static int M_StringWidth(const std::string &string);
static int M_StringHeight(const std::string &string);
static int M_BigStringWidth(const std::string &string);
...
static void M_SetInputString(const std::string &string, int len);
...
void M_ChooseSkill(int choice) {
    G_DeferedInitNew(static_cast<skill_t>(choice),1);
    M_ClearMenus();
...
void M_Network(int choice) {
    ...
    inputString = m_playername.string;
}
...
void M_SaveSelect(int choice) {
    saveSlot = choice;
    inputString = savegamestrings[choice];
...
void M_ReadSaveStrings(void) {
    ...
    for(i = 0; i < load_end; i++) {
        ...
        std::ifstream handle(P_GetSaveGameName(i), std::ios::binary);
        if(!handle.is_open()) {
            dstrcpy(&savegamestrings[i][0],EMPTYSTRING);
            DoomLoadMenu[i].status = 0;
            continue;
        }
        savegamestrings[i].resize(MENUSTRINGSIZE);
        handle.read(const_cast<char*>(savegamestrings[i].data()), MENUSTRINGSIZE);
...
static void M_SetInputString(const std::string &string, int len) {
    ...
    inputCharIndex = inputString.size();
    inputMax = len;
}
    ...
    if(inputEnter) {
        ...
        case KEY_ENTER:
            ...
            if(currentMenu == &NetworkDef) {
                ...
                savegamestrings[saveSlot] = inputString;
