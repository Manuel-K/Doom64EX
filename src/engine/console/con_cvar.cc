...
float CON_CvarValue(const std::string &name) {
    cvar_t *var;
...
std::string CON_CvarString(const std::string &name) {
    cvar_t *var;
...
void CON_CvarAutoComplete(const std::string &partial) {
    ...
    for(cvar = cvarcap; cvar; cvar = cvar->next) {
        if(partial == cvar->name.substr(partial.size())) {
            if(!match) {
                match = true;
                CON_Printf(0, "\n");
            }
            ...
            // setup spacing
            spacinglength = 24 - dstrlen(cvar->name.c_str());
            spacing = new char[spacinglength + 1];
