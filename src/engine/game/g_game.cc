...
static CMD(SpawnThing) {
    ...
    thing = P_SpawnMobj(x, y, z, static_cast<mobjtype_t>(id));
...
void G_DoLoadLevel(void) {
    ...
    if(!netgame) {
        gameskill   = (skill_t)sv_skill.value;
...
void G_RunTitleMap(void) {
    ...
    demobuffer = new byte[0x16000];
    std::fill_n(demobuffer, 0, 0x16000);
    demo_p = demobuffer;
