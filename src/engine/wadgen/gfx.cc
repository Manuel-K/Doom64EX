...
void Gfx_CreateEXLump(gfxEx_t * pcGfx, gfxRom_t * romGfx)
{
    ...
    pcGfx->data = new byte[pcGfx->width * pcGfx->height];
...
void Gfx_GetCloudLump(gfxEx_t * pcGfx)
{
    ...
    romWadFile.lumpcache[l] = reinterpret_cast<byte*>(Wad_GetLump(romWadFile.lump[l].name, false));

    ZeroMemory(pcGfx, sizeof(gfxEx_t));

    pcGfx->width = 64;
    pcGfx->height = 64;
    pcGfx->data = new byte[4096];
